...
- 일반 좀비 피격과 같은 4 units/s·160ms 넉백 및 300ms 재접촉 쿨다운
...
- `DOGE_KNOCKBACK_SPEED`와 `DOGE_KNOCKBACK_MS`가 일반 좀비 피격과 같은 `4 units/s`, `160ms`인지 확인한다.
- 방향, 피해 없음, 센서와 300ms 재접촉 쿨다운 계약은 기존 테스트로 회귀 검증한다.

### 실행 결과

- 집중 테스트: `dogeEscape`, `DancingDogeEvent`, `Player`와 B01 관련 테스트 5파일, 25개 통과.
- 프로덕션 빌드, branch guard, legacy B02 source·artifact gate 통과.
