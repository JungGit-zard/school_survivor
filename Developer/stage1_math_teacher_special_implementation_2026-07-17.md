...
- 충격 프레임에 보스 반경 1.05 units(0.2625 블록) 안의 살아 있는 좀비를 바깥쪽으로 밀어낸다.
...
- 이전 2.4 units(0.6 블록) 광역 반경을 1.05 units(0.2625 블록)로 축소했다.
- B01 삼각자 충돌 키포즈의 시각 도달거리 약 0.9 units와 플레이어 충돌 반경(0.192 units)만 고려해, 플레이어와 일반 좀비 모두 삼각자에 거의 닿아야 판정한다.
- 피해 비율, 준비·회복 시간, 좀비 밀치기 속도·시간, 모델과 애니메이션은 변경하지 않았다.
