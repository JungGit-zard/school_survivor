...
test('normalizes the limited ranking run payload', () => {
  assert.deepEqual(
    normalizeRun({ runId: 'run_20260712_001', stageId: 'stage2', score: 396, timeMs: 240000, cleared: true }),
    { runId: 'run_20260712_001', stageId: 'stage2', score: 396, timeMs: 240000, cleared: true },
  )
})

test('accepts stage3 and stage4 submissions (B1: server whitelist)', () => {
  // stage3 clear 240s: base = 240 + 120 + 30 = 390, +boss floor(390*0.2)=78 -> 468
  assert.doesNotThrow(() =>
    normalizeRun({ runId: 'run_stage3_0001', stageId: 'stage3', score: 468, timeMs: 240000, cleared: true }),
  )
  // stage4 clear 240s: base = 240 + 180 + 30 = 450, +boss floor(450*0.2)=90 -> 540
  assert.doesNotThrow(() =>
    normalizeRun({ runId: 'run_stage4_0001', stageId: 'stage4', score: 540, timeMs: 240000, cleared: true }),
  )
})
...
test('rejects tampered scores above the honest per-run maximum (M5)', () => {
  // stage2, 95s, cleared: max = (95+60+30) + floor(185*0.2)=37 -> 222. 840 is impossible.
  assert.throws(
...
test('accepts the exact honest maximum score for each stage (M5 boundary)', () => {
  ...
  assert.equal(maxLegitScore('stage4', 240000, true), 540)
  assert.doesNotThrow(() =>
...
test('records one run once and atomically projects daily, weekly, stage-daily, and stage-weekly totals', () => {
  const args = {
    uid: 'google-user-123',
    displayName: '학생A',
    run: normalizeRun({ runId: 'run_20260712_001', stageId: 'stage2', score: 396, timeMs: 240000, cleared: true }),
    seasonId: 'season-001',
  ...
  assert.deepEqual(publicData.global.daily['2026-07-12'].entries[key], {
    displayName: '학생A', score: 396, timeMs: 240000, cleared: true, updatedAt: args.nowMs,
  })
  assert.equal(publicData.global.weekly['2026-07-06'].entries[key].score, 396)
  assert.equal(publicData.stage.stage2.daily['2026-07-12'].entries[key].score, 396)
  // L8: stage-scoped weekly board is now projected too.
  assert.equal(publicData.stage.stage2.weekly['2026-07-06'].entries[key].score, 396)
  ...
  assert.equal(duplicate.state.public['season-001'].global.daily['2026-07-12'].entries[key].score, 396)
  assert.equal(duplicate.state.public['season-001'].stage.stage2.weekly['2026-07-06'].entries[key].score, 396)
})
