...
//
// 주의(정규화 모델): stageExpectedBaseJarmobHp는 30초 격자(t=0,30,…,210)만 표본으로 삼아
// 72~90·96~120·192~208·224~240(총 56초)을 아예 못 본다. 격자에 안 걸리는 구간에만 부하를 실으면
// 격자 총량은 그대로인데 실런타임 전달량만 올라간다. 그래서 이 재배분은 "격자 합 상승률 ≈ 지속시간
// 가중 합 상승률"이 되도록 잡았다: 격자 +10.00% / 지속시간 가중 +10.61% / 실런타임 몬테카를로 +9.42%.
// 회귀 방어는 stageDurationWeightedJarmobHp 단언(Enemies.test.jsx)이 맡는다.
// 72~90과 90~96을 같은 target·weights로 맞춘 건 t=90 표본 하나가 6초짜리 phase를 대표하던
// 왜곡을 없애기 위해서다. 총 HP는 STAGE_JARMOB_TOTAL_HP_FACTOR(Enemies.jsx)가 결정한다.
export const STAGE2_WAVE_PHASES = [
  ...
  // 1:36–2:00 탱커 벽 + E04 캡 2기 승격(96s). 구조값 불변 — 원래도 골짜기가 아니었다.
  { start:  96, end: 120, target: 19, weights: { E01: 0.20, E02: 0.65, E04: 0.15 } },
  // 2:00–2:24 차저 E05 도입 — 골짜기 바닥이었다. E05 비중과 E02 소량으로 15 HP/s대까지 올린다.
  //           (보스는 여기가 아니라 170~190초에 나온다 — 파일 상단 주의 참조)
  { start: 120, end: 144, target: 26, weights: { E01: 0.38, E03: 0.30, E05: 0.22, E02: 0.075, E04: 0.025 } },
  // 2:24–2:48 보스 직전 빌드업 — 이전 E03 0.58 편중(가벼운 러너 떼)을 걷어내고 E05/E02로 무게를 준다.
  { start: 144, end: 168, target: 28, weights: { E03: 0.42, E05: 0.30, E02: 0.14, E04: 0.14 } },
  ...
  // 3:44–4:00 탈출 스프린트 — 구조값 불변(포탈 210s 이후라 재배분 대상 아님).
