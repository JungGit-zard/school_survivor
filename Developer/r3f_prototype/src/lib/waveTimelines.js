...
const BASE_WAVE_PHASES = [
  ...
  // 2:24–2:30 돌진 본격 도입
  { start: 144, end: 150, target: 29, weights: { E01: 0.50, E03: 0.05, E02: 0.30, E05: 0.15 } },
  // 2:30–2:48 보스 구간 초입
  { start: 150, end: 168, target: 29, weights: { E01: 0.50, E03: 0.05, E02: 0.30, E05: 0.15 } },
  ...
  // 3:12–3:28 후반 구성 1 (잡몹+탱커) — 보스 구간 여부는 등장 시각(2:30)에서 파생
...
//
// ★ 보스 등장 시각: STAGE2_BURST_EVENTS의 B02는 150초에 런타임에서도 그대로 발화한다.
//   따라서 150~192 phase가 보스 창이다. 이 구간은 bossPressure(Enemies.jsx)로 E04 발사까지
...
//
// E04(원거리) weights는 전 구간 그대로 뒀다.
// 주의(2026-08-19 정정): 예전 주석은 "동시 등장이 getE04Cap으로 하드캡된다"를 근거로 삼았지만
// getE04Cap(stage2ProjectileRules.js)에는 런타임 호출자가 없다 — 죽은 코드다. 사수 수를 제한하는 장치는
// 게임 어디에도 없고, 실제 상한은 enemySimulation.js isE04FireAllowed의 비행 투사체 6발(E04_MAX_PROJECTILES)
// 하나뿐이다. 투사체 수명 3.2s / 쿨다운 2.2s이므로 포화 사수 수 = 6/3.2×2.2 = 4.13기이고,
// 그 위로는 사수를 늘려도 발사 압박이 늘지 않는다(자세한 계산은 burstEvents.js STAGE4 주석).
...
const BASE_STAGE2_WAVE_PHASES = [
  ...
  // 1:36–2:00 탱커 벽 + E04 비중 상승(96s. "캡 2기 승격"이라 적혀 있었으나 그 캡은 죽은 코드다 — 위 주석 참조).
  //           2026-08-07: 여기는 골짜기가 아니라 스1 대비 1.7배 봉우리라
  ...
  // 2:24–2:30 보스 직전 빌드업 — 가벼운 러너 떼(E03)를 더 걷어내고 E05/E02로 무게를 준다.
  //           2026-08-07: 스1 140~160s 대비 0.77배였던 구간. E03 0.42→0.32, E05 0.30→0.35, E02 0.14→0.19로 +14%.
  { start: 144, end: 150, target: 28, weights: { E03: 0.26, E05: 0.38, E02: 0.22, E04: 0.14 } },
  // 2:30–2:48 ★ 보스 창 + 거대 E06 = 스테이지 최대 봉우리.
  //           구조값 불변. bossPressure로 E04 발사가 막히는 구간이라 잡몹 부하를 깎으면 보스전이 헐거워진다.
  { start: 150, end: 192, target: 29, weights: { E02: 0.66, E04: 0.16, E06: 0.18 } },
...
//            (c) 체육교사 B03 단일 보스(burstEvents에서 파생, 150초 고정).
...
// 조기 도입 사슬은 버스트 시간표 재배열 후 E04@40·E05@72로 맞춘다.
// (2026-08-19: E06@110×1은 25초 반복 보강이 그 창을 채우게 되면서 삭제됐다 — 거대는 184초 pincer ×2 한 곳뿐이다.)
...
const BASE_STAGE3_WAVE_PHASES = [
  ...
  // 2:12–2:30 보스 직전 피크 — 최대 복합. E04 비중 상승(132. 예전의 "cap 3 승격"은 죽은 getE04Cap 인용이었다).
  //           target 30→27(위와 같은 이유 + 스택 과부하 방지)
  { start: 132, end: 150, target: 21, weights: { E01: 0.20, E03: 0.10, E02: 0.20, E05: 0.24, E06: 0.14, E04: 0.12 } },
  // ── 보스: 체육교사 B03 단일(150초). 더블 보스 B02+B01은 2026-07-21 폐기됐다(burstEvents.js). ──
...
// 상승은 마릿수가 아니라 (a) 원거리 지속 압박, (b) 단일 보스 B04(150초 등장) 스포트라이트에서 온다.
...
// weights 합 = 1.00. 보스@150 이후 잡몹 target 급감(16~20, 보스 집중), 215~240 탈출 스프린트는 E01 다수.
const BASE_STAGE4_WAVE_PHASES = [
  ...
  // 2:08–2:30 보스 직전 피크 정점 — 차저 비중↑
  { start: 128, end: 150, target: 28, weights: { E01: 0.20, E03: 0.08, E02: 0.22, E05: 0.24, E06: 0.12, E04: 0.14 } },
  // ── 주방장 보스 B04@150(burstEvents). isBossPhase는 150에서 파생 ──
  // 2:30–2:40 보스 구간 1 — 잡몹 target 급감(보스 집중). 원거리 E04만 고비중 유지 = 시그니처(보스 구간 안전지대 없음)
  { start: 150, end: 160, target: 16, weights: { E01: 0.34, E02: 0.26, E05: 0.18, E04: 0.22 } },
...
// 2026-08-17: 빈 앵커 168초에 원거리 포위(ring E04)를 새로 넣었다 — 스4 시그니처
// "안전지대 소멸"을 형태 버스트로 문자 그대로 구현한 것이라 예고가 특히 중요하다.
// 2026-08-19: 그 포위는 E04×9 → ×4로 줄었다. 투사체 슬롯 6·쿨다운 2.2s·수명 3.2s가 만드는
// 포화 사수 수가 4.13기라, 9기는 5기분이 사선을 못 얻고 근접도 안 하는 순수 HP 낭비였다.
// 예고 라벨·시각은 그대로다 — 화면에 보이는 "사방에서 날아온다"는 4기로도 동일하게 성립한다.
export const STAGE4_SPAWN_TELEGRAPHS = [
  ...
  { sec: 168, leadSec: 2.5, label: '사방에서 급식판이 날아온다' }, // ring   E04×4 (원거리 포위 = 안전지대 소멸, 포화점)
