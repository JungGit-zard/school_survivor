...
describe('단(segment)별 유효 사거리·유효 위력', () => {
  ...
  it('단 0→7에서 위력이 5.4 → 5.4×1.84로 자란다', () => {
    const w = base()
    expect(bikittySegmentDamage(w, 0)).toBeCloseTo(5.4, 10)
    expect(bikittySegmentDamage(w, 7)).toBeCloseTo(5.4 * 1.84, 10)
    expect(bikittySegmentDamage(w, 7)).toBeCloseTo(9.936, 10)

    // 가산 배율이므로 단당 증가폭은 항상 base damage × 0.12로 일정하다.
    for (let s = 1; s <= 7; s += 1) {
      expect(bikittySegmentDamage(w, s) - bikittySegmentDamage(w, s - 1)).toBeCloseTo(5.4 * 0.12, 10)
    }
  })
  ...
  it('범위를 벗어난 단은 0~7로 clamp된다', () => {
    ...
    expect(bikittySegmentDamage(w, -1)).toBeCloseTo(5.4, 10)
  })
...
describe('사이클 검산 (기획서 §1 위력 계산)', () => {
  it('2026-09-12 변경 전의 10% 값에서 8단과 snap을 합친 사이클 피해를 계산한다', () => {
    const w = base()
    ...
    expect(bikittyCycleDamage(w)).toBeCloseTo(5.4 * 11.36 + 9, 6)
    expect(bikittyCycleDamage(w)).toBeCloseTo(70.344, 6)

    // 사이클 시간 = 8 × 1200ms + reload 1200ms = 10800ms
    expect(bikittyCycleMs(w)).toBe(10800)

    expect(bikittyCycleDps(w)).toBeCloseTo(6.51, 2)
  })
