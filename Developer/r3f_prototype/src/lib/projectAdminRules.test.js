...
describe('Realtime Database project master rule', () => {
  it('keeps root denied and grants owner-or-verified-Google-master access only to user and Studio paths', () => {
    ...
    expect(rules.match(/auth\.token\.email === 'zard5388@gmail\.com'/g)).toHaveLength(3)
    expect(rules.match(/auth\.token\.email_verified === true/g)).toHaveLength(3)
    expect(rules.match(/auth\.token\.firebase\.sign_in_provider === 'google\.com'/g)).toHaveLength(3)
  })
})
