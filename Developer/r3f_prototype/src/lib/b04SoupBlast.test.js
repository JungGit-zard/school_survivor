...
describe('B04 국물 대폭발', () => {
  ...
  it('세 원은 경계·장애물을 피하고 서로 겹치지 않으며 중심 안전 경로를 남긴다', () => {
    const circles = getB04SoupBlastCircles({ player: { x: 0, z: 0 }, halfX: 9.36, halfZ: 16, obstacles: [{ x: 3.2, z: 0, halfX: 1, halfZ: 1 }] })
    expect(circles).toHaveLength(3)
    expect(circles.every((circle) => Math.abs(circle.x) < 9.36 && Math.abs(circle.z) < 16)).toBe(true)
