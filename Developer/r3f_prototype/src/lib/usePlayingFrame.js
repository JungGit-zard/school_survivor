...
export function usePlayingFrame(callback) {
  ...
  if (clockRef.current === null) clockRef.current = createGameplayFixedStepClock()
  useFrame((state, delta) => {
    if (!shouldRunGameFrame(useGameStore.getState().phase)) return
    runGameplayFixedSteps(clockRef.current, delta, (fixedDelta) => callback(state, fixedDelta))
  })
}
