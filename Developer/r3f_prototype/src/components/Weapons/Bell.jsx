...
export function BellShockwave() {
  ...
  usePlayingFrame(({ clock }) => {
    ...
    // volume 0.55: 벨은 발사음과 타격음이 같은 프레임에 나가는 유일한 무기다.
    // 두 음원 모두 generate_sfx.mjs의 mix()가 피크 1.0으로 정규화하므로, 예전처럼
    // bellFire를 기본 1.0으로 두면 순간 합이 1.0+0.45=1.45(풀스케일 +3.2dB)가 되어
    // destination에서 클리핑된다. 0.55+0.45=1.00이라 합이 정확히 풀스케일에 닿는다.
    // 2026-08-22부터 SFX_MASTER_VOLUME이 전역을 0.5로 눌러 여유가 생겼지만, 그건 모든
    // 소리에 똑같이 걸리는 배율이라 벨 안에서의 발사:타격 비율은 여전히 여기서 잡아야 한다.
    // 부수 효과도 옳다 — 헛친 펄스는 0.55 링만, 맞힌 펄스는 1.00으로 울려
    // 명중/빗나감 구분이 오히려 또렷해진다.
