...
export default function ZombieInstanceLayer({ resetKey }) {
  ...
  useFrame((_,delta) => {
    ...
    for(let i=0;i<200;i++) {
      ...
      if(!tier)continue
      const bodyVisible=timer>=SPAWN_REVEAL_MS&&((type>=1&&type<=8)||type===13||type===14); const smokeVisible=timer>=0&&timer<SPAWN_SMOKE_MS
      if(bodyVisible){const renderSlot=bodyCount++;const rootMatrix=studio.current.rootMatrices[type]||IDENTITY;const scale=(pool.visualScale[i]||1)*.333;const state=phase(pool,i);const time=getPooledEnemyAnimationTime(timer,tier);const pal=TYPE_PALETTES[type];const parts=type===13?RZT:type===14?RZG:RUN_TYPES.has(type)?RUN:STANDARD;const offset=type===13?RZT_OFFSET:type===14?RZG_OFFSET:RUN_TYPES.has(type)?STANDARD.length:0
