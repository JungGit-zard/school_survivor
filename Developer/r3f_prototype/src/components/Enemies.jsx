...
export default function Enemies() {
  ...
  pooledHitBridgeRef.current = (index, generation, damage, impact) => {
    ...
    if (critical.isCritical) emitSfx({ id: 'criticalHit', volume: strongCritical ? 1.8 : 1.52 })
  ...
  runtimeQueueRef.current.processScheduled = (kind, a, b) => {
    ...
    if (kind === SCHEDULE_GOLD) {
      ...
      if (evt.formation === RUN_ZOMBIE_CREW_FORMATION) {
        ...
        const batch = createRunZombieCrewEntries(cache.bounds, Math.random, cache.obstacles, { x: playerPos.x, z: playerPos.z })
