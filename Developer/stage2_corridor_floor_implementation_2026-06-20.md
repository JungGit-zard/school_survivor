...
Developer/r3f_prototype/src/lib/playerMovementBounds.js
Developer/r3f_prototype/src/lib/stage2CorridorWall.js
...
- Stage 2 상단 교실 문/창문 이미지는 처음 축소안보다 2배 큰 `2/5` 표시 크기로 키우고, 가로 방향으로 5회 반복되게 했다.
- Stage 2 상단 교실 문/창문 이미지의 아랫선을 플레이어가 넘지 못하는 벽으로 연출한다.
- 캐릭터의 보이는 몸이 벽 이미지 안으로 겹쳐 들어가지 않도록, 플레이어 정지선은 이미지 아랫선보다 약간 아래에 둔다.
