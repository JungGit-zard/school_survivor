...
- **3시간마다 1개 엔트리** 추가. 변경 사항이 있든 없든, 세션이 이어지는 동안 3시간 단위 엔트리는 반드시 남긴다.
...
세션이 이어지는 동안 3시간이 지날 때마다 `SESSION_MEMORY.md`의 **끝에 새 엔트리를 append**한다.

변경 사항이 없거나 파일 수정이 없었더라도 엔트리는 생략하지 않는다. 이 경우 해당 항목에 `변경 없음`, `새 파일 없음`, `검증 없음`처럼 명확히 적어 세션 공백도 기록으로 남긴다.
...
- **이번 3시간 작업 / 대화**: 중요한 결정사항, 사용자 지시, 정책 변경. 없으면 `변경 없음`이라고 적는다.
- **생성 / 수정 / 이동 파일 목록**과 각 파일의 목적. 없으면 `새 파일 없음` 또는 `파일 변경 없음`이라고 적는다.
- **명령 로그**: 이번 3시간 동안 실행한 모든 명령을 실행 순서대로 원문 그대로 적는다. 명령이 없으면 `실행 명령 없음`이라고 적는다.
- **명령 / 빌드 / 테스트 / 브라우저 검증 결과** + 스크린샷 경로. 없으면 `검증 없음`이라고 적는다.
- **확정된 룰 / 정책 변경**. 없으면 `새 정책 변경 없음`이라고 적는다.
- **미해결 이슈 + 다음 단계** + 다음 세션이 가장 먼저 읽어야 할 항목. 없으면 `미해결 이슈 없음`이라고 적는다.
