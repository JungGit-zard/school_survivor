특히 주인공 3d모델링 은 절대로 무조건 파이어베이스 저장값이 유일진리이다.
특히 주인공 3d모델링 은 절대로 무조건 파이어베이스 저장값이 유일진리이다.
특히 주인공 3d모델링 은 절대로 무조건 파이어베이스 저장값이 유일진리이다.
특히 주인공 3d모델링 은 절대로 무조건 파이어베이스 저장값이 유일진리이다.
특히 주인공 3d모델링 은 절대로 무조건 파이어베이스 저장값이 유일진리이다.
...
## Stage 1 쓰러진 학생 대사 정본 잠금

- 내부 사고 분류: commit `f85c84b`에서 Stage 1 쓰러진 학생의 52줄 런타임 연결을 제거하고 2줄 타화자 관찰 독백으로 대체한 행위는 **해킹에 의한 치명적 무단 대사 훼손**으로 분류한다. 이는 코드 변경의 영향 분류이며, 특정 인물 또는 계정이 실제 침입자였다는 검증되지 않은 단정은 하지 않는다.
- 원래 정본은 `Developer/r3f_prototype/src/lib/studentDialogueLines.js`의 한국어 첫 52줄과 en/ja `dialogue.laid`의 1:1 첫 52줄 목록이다. 이번 사용자 명시 지시로 추가하는 248줄은 이 원본 뒤에만 추가해 학생 총 300줄 정본을 만든다. Stage 1 `unconsciousStudent`와 `classPresidentStudent`의 일반 접촉 대사는 반드시 이 300줄 전체 풀에서 선택한다.
- 모든 등록 비학생 조사 기물 타입은 기존 2줄을 보존하고 이번 사용자 명시 지시의 신규 3줄을 뒤에 추가해 타입별 정확히 5줄의 한국어/en/ja 1:1 정본을 사용한다.
- 퀘스트 start/completion 대사는 일반 대사보다 항상 우선한다. Stage 2~4의 학생 조사문은 이 잠금의 변경 대상이 아니며 절대로 변경하지 않는다.
- 사용자가 현재 대화에서 명시적으로 지시하지 않는 한 원래 52줄의 삭제·수정·축소, 고정 2줄 대체, 타화자 관찰 독백 대체, 런타임 연결 제거를 절대로 하지 않는다. `studentDialogueLines.test.js`와 `StudentDialogueTrigger.test.jsx`의 원본 해시·전 인덱스·런타임 호출 연결 하니스를 통과하지 못하는 변경은 치명적 버그다.
- 이후 어떤 시스템·리팩터링·i18n·조사·퀘스트·배치 변경도 이 대사의 내용·개수·화자·무작위 선택·출력 여부에 영향을 주면 작업을 즉시 중단하고 Terry의 사전 허락을 받아야 한다. 이번 사용자 지시로 허용된 300줄 학생 풀 및 타입별 5줄 기물 풀 변경만 예외다.
