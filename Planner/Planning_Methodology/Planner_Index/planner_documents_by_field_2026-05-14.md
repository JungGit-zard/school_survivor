...
3. `Planner/B.게임기획,밸런스 구현/B-3 스테이지진행과 몬스터 등장구현/Stage1_Balance/stage1_replan_2026-05-06.md`
   - 1스테이지 최신 재기획 기준.
   - 2026-05-09 부록의 1스테이지 탄환 몬스터 금지 규칙 포함.

4. `Planner/B.게임기획,밸런스 구현/B-3 스테이지진행과 몬스터 등장구현/Stage1_Balance/stage1_reverse_design_current_2026-05-09.md`
   - 현재 구현 상태 기준 역기획 문서.
...
| 문서 | 성격 | 우선도 | 핵심 내용 |
|---|---|---:|---|
| `Rewards_Drops/dual_drop_system_2026-05-08.md` | XP/골드 이원화 최신 기준 | 매우 높음 | 교과서 XP, 황금 코인 분리 |
...
```text
Planner/Planning_Methodology/Planner_Index/planner_disposal_log_2026-05-14.md
```
...
| 서브에이전트로 기획 평가 | `Planning_Methodology/Subagent_Workflow/subagent_planner.md` |
| 파기된 문서 확인 | `Planning_Methodology/Planner_Index/planner_disposal_log_2026-05-14.md` |
...
## 14. 추가 정리 (2026-05-24 — Planning_Methodology 통합)

기획용 서브에이전트·CE 기록·브레인스토밍/검수 방법론 문서를 한 폴더로 모았다.

```text
Planner/
  current_game_rules.md
  Planning_Methodology/                 # ← 신설
    README.md                            # 폴더 안내
    Subagent_Workflow/                   # 기존 Subagent_Workflow에서 이동
      subagent_planner.md
      subagent_validation_pipeline.md
    Compound_Engineering/                # Index에서 이동
      compound_engineering_folder_structure_2026-05-16.md
    Planner_Index/                       # Index에서 이동
      planner_documents_by_field_2026-05-14.md
      planner_disposal_log_2026-05-14.md
    Brainstorm_Cases/                    # Essential_game_plan에서 이동
      title_screen_multi_agent_concept_review_2026-05-23.md
    Major_Review_Point/                  # 기존 Major_Review_Point에서 이동
      5.16_게임기술스택,코드_중간점검.md
      5.16_플레이테스트_XP경제_실측.md
  Index/                                 # 잔존: project_name_rename_review만 남음
  Essential_game_plan/                   # 잔존: 실제 feature 기획 5종
  B.게임기획,밸런스 구현/                # 잔존: 게임 콘텐츠 기획
  Ref_Vampire_GameDesign/                # 잔존
  Tech_plan/                             # 잔존
```

이동 사유: 방법론(메타)과 콘텐츠(실제 기획)를 분리해 두 종류의 문서를 헷갈리지 않게 한다. 본 문서(planner_documents_by_field) 자체도 색인 메타이므로 함께 이동.

`SESSION_MEMORY.md`, `CEO/docs/*`, `Planner/B.*`, `Planner/Index/project_name_rename_review_2026-05-17.md` 안의 옛 경로 참조는 시점 기록이므로 갱신하지 않는다.
