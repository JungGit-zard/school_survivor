...
> 🔄 **2026-08-23 — Stage 4 가로 크기 30% 재확장:** Stage 4 급식실의 `mapHalfX`는 직전 **7.2→9.36 units**로 정확히 30% 늘리고 전체 가로는 **18.72 units**로 한다. `mapHalfZ=16` units, 타일 1칸 `0.8×0.8` units, 바닥 깊이 32 units는 변경하지 않고, Stage 4 소품은 새 경계 안에서 관통·겹침·플레이어 전용 틈이 없도록 X 위치만 재배치하며 개수·타입·Y/Z·회전·스케일·props는 유지한다. Stage 1∼3은 변경하지 않는다.

> 🔄 **2026-08-09 — Stage 1 0:40 녹색좀비·웃는좀비 추가 고정 스폰:** Stage 1은 기존 웨이브/버스트/110초 보강/150초 보강/보스/마틸다/오버타임을 보존한 상태에서 정확히 **0분 40초 = 40초**에 녹색좀비 `E01` **5마리**와 웃는좀비 `E07` **3마리**를 추가 고정 스폰한다. Stage 2~4에는 이 40초 보강을 추가하지 않는다. 런타임은 `getRuntimeBurstEventsForStage('stage1')` → `stageRuntime.burstEvents` → `SCHEDULE_BURST` → pooled `addEnemies(..., true, spawnToken)` 경로에서 실제 소비해야 하며, 적 스탯, AI, 모델, 오디오, Firebase, Graphics Studio, 타이틀, 스테이지 길이, 보스/마틸다/오버타임 타이밍은 변경하지 않는다.

> 🔄 **2026-08-09 — Stage 1 2:30 웃는좀비·녹색좀비 보강:** Stage 1은 기존 웨이브/버스트/보스/마틸다/오버타임을 보존한 상태에서 정확히 **2분 30초 = 150초**에 웃는좀비 `E07` **5마리**와 녹색좀비 `E01` **5마리**를 추가 스폰한다. 이 150초 보강은 Stage 2~4에는 추가하지 않으며, 기존 전 스테이지 **1분 50초 = 110초** `E07` 3마리 + `E02` 3마리 보강은 그대로 유지한다. 적 스탯, AI, 모델, 오디오, Firebase, Graphics Studio, 타이틀, 스테이지 길이, 보스/마틸다/오버타임 타이밍은 변경하지 않는다.

> 🔄 **2026-08-09 — 마틸다 실제 몸 접촉 즉사 1000ms 지연:** 플레이어가 마틸다 실제 몸 CuboidCollider 기반 회전 박스에 처음 접촉하면 즉시 죽이지 않고, 게임플레이 delta 기준 정확히 **1000ms = 1초** 뒤 `killPlayer('matilda')`를 1회 호출한다. 접촉이 유지되는 동안만 카운트다운을 누적하며, 접촉이 끊기면 즉시 **1000ms**로 초기화하고 다시 연속 접촉해야 한다. 판정 반extent는 기존 `getMatildaBodyHalfExtents`/`isMatildaBodyContact` 정본을 유지한다: 마틸다 B01 scale 2.00 기준 halfX **0.3733 units (0.0933 블록)**, halfZ **0.2667 units (0.0667 블록)**, 플레이어 접촉 반extent **0.136 units (0.034 블록)** 포함 접촉 한계는 x축 **0.5093 units (0.1273 블록)**, z축 **0.4027 units (0.1007 블록)**이다. paused/levelup/gameover 등 `playing` 외 phase에서는 카운트다운이 진행되지 않으며 마틸다 스탯/AI/모델/오디오/타이틀/Firebase는 변경하지 않는다.

> 🔄 **2026-08-09 — 전 스테이지 5:00 이후 혼합 일반 좀비 반복 보강/150 상한:** Stage 1~4 모든 플레이어블 스테이지는 정확히 **5분 00초 = 300초**부터 시작해 이후 **30초** 간격(**330초, 360초, ...**)으로 일반 좀비 혼합 보강을 **30마리 요청**한다. 동시 좀비 상한은 pooled active + special boss/Matilda active + deferred pooled queue 합산 기준 **150마리**이며, 남은 슬롯이 30 미만이면 그 수만큼만 추가하고 기존 좀비는 삭제하지 않는다. Stage 1 풀은 `E01,E02,E03,E05,E06,E07`로 **E04 제외**를 유지하고, Stage 2~4 풀은 `E01`~`E07`이다. 보스, RZ 이벤트 적, 마틸다, 도지는 포함하지 않으며 기존 웨이브·110초 보강·보스·마틸다·스탯·모델·AI·Firebase·Studio·타이틀·오디오는 변경하지 않는다.

> 🔄 **2026-08-09 — 전 스테이지 1:50 웃는좀비·탱커 보강:** Stage 1~4 모든 플레이어블 스테이지는 기존 웨이브/버스트를 보존한 상태에서 정확히 **1분 50초 = 110초**에 웃는 좀비 `E07` **3마리**와 탱커 `E02` **3마리**를 추가 스폰한다. 스테이지 길이, 탈출 포탈, 보스 타이밍, 마틸다, 기존 웨이브/버스트, 적 스탯·AI·모델, Firebase, Graphics Studio, 타이틀, 오디오는 변경하지 않는다.

> 🔄 **2026-08-18 — 전 스테이지 마틸다 3:30 등장:** Stage 1~4 모든 플레이어블 스테이지에서 마틸다 물리 액터/AI는 **3분 30초 = 210초**에 정확히 등장한다. 기존 5초 경고·대사 유예는 유지하여 경고/상태 전환은 **3분 25초 = 205초**, 실제 Enemies 물리 스폰은 **210초**다. 스테이지 길이 **240초**, 탈출 포탈 **210초**, 보스 타이밍, 웨이브, 마틸다 스탯·AI·모델·대사·오디오, Firebase, Graphics Studio, 타이틀은 변경하지 않는다.

> 🔄 **2026-08-23 — 커터칼 치명타 확률:** `boxCutter` 기본 치명타 확률은 **0.33 (33%)**이다. 런 중 `boxCutterCrit`는 1회마다 **+0.04 (4 percentage points)**를 적용하며, 영구 치명타 업그레이드 최대 **+0.08**과 런 강화 4회 **+0.16**를 모두 그대로 반영하는 상한은 **0.57**이다 (`0.33 + 0.08 + 0.04 × 4 = 0.57`). 피해·쿨다운·사거리·폭·넉백·치명타 배율·다른 무기의 치명타 수치는 변경하지 않는다.
