...
> 🔄 **2026-08-23 — Stage 4 가로 크기 절반:** Stage 4 급식실의 `mapHalfX`는 **14.4→7.2 units**로 정확히 절반으로 줄인다. `mapHalfZ=16` units, 타일 1칸 `0.8×0.8` units, 바닥 깊이 32 units는 변경하지 않고, Stage 4 소품은 새 경계 안에서 관통·겹침·플레이어 전용 틈이 없도록 X 위치만 재배치하며 개수·타입·Y/Z·회전·스케일·props는 유지한다. Stage 1∼3은 변경하지 않는다.
